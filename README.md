...
使用 Playwright `page.screencast` 录制 WebM。页面创建后先启动录制，确认收到首帧后才跳转目标页面，避免漏掉开场动画；10 秒内未收到首帧则终止，不打开业务页面。原始文件暂存于 `output/videos/.pending/`。产品选择完成后等待 2–3 秒，在裁剪点直接停止录制；浏览器随后继续监控 `success`，但这段等待不会进入视频。裁剪时长从首帧时间计算，包含导航前的就绪画面。流程结束后，成功视频由 `ffmpeg` 按裁剪时长转码为 MP4 并输出到：
...
9. 点击完善信息
10. 触发协议弹窗后，40% 用户停留 0.5–1.5 秒再浏览，60% 用户等待 1–3 秒后直接同意；浏览超过约 2 秒可能出现蒙层。关闭蒙层后，25% 直接同意、25% 继续浏览当前协议 2–5 秒、50% 停留 0.5 秒后切换 TAB；切换后 20% 直接同意，80% 继续浏览不同 TAB，最终浏览 1/2/3/4 个 TAB 的用户约占 60%/25%/10%/5%。TAB 浏览期间再次出现蒙层时会立即暂停滚动和浏览计时，关闭蒙层后从当前 TAB 继续，单个 TAB 最多处理 3 次二次蒙层
11. 触发产品弹窗后随机浏览等待 1–5 秒；等待超过约 2 秒可能出现蒙层，但不阻断产品按钮；用户关闭蒙层后仍可继续浏览再点击
12. 产品选择后等待 2–3 秒并停止 screencast；继续监控成功 Toast，成功后按裁剪点用 ffmpeg 转码生成 MP4
...
3. 点击完善信息
4. 触发协议弹窗后，40% 用户停留 0.5–1.5 秒再浏览，60% 用户等待 1–3 秒后直接同意；浏览超过约 2 秒可能出现蒙层。关闭蒙层后，25% 直接同意、25% 继续浏览当前协议 2–5 秒、50% 停留 0.5 秒后切换 TAB；切换后 20% 直接同意，80% 继续浏览不同 TAB，最终浏览 1/2/3/4 个 TAB 的用户约占 60%/25%/10%/5%。TAB 浏览期间再次出现蒙层时会立即暂停滚动和浏览计时，关闭蒙层后从当前 TAB 继续，单个 TAB 最多处理 3 次二次蒙层
5. 触发产品弹窗后随机浏览等待 1–5 秒；等待超过约 2 秒可能出现蒙层，但不阻断产品按钮；用户关闭蒙层后仍可继续浏览再点击
6. 产品选择后等待 2–3 秒并停止 screencast；继续监控成功 Toast，成功后按裁剪点用 ffmpeg 转码生成 MP4
