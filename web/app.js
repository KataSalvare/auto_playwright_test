(() => {
  ...
  const state = { mode: 'auto', presetId: config.presets?.[0]?.id || '', currentUrl: '', parsedParams: {}, validation: null, showAllParsed: false, history: [], run: null };
  ...
  function validateAndRender(rawUrl, sourceLabel) {
    ...
    const validation = validateParams(url); state.run = null; clearRun(); state.currentUrl = url.toString(); state.parsedParams = Object.fromEntries([...url.searchParams.entries()]); state.validation = validation; state.showAllParsed = false; $('#validationEmpty').hidden = true; $('#validationContent').hidden = false; $('#urlBox').textContent = state.currentUrl; $('#resultHeadline').textContent = validation.allValid ? sourceLabel : '链接需要修正'; $('#resultSubline').textContent = validation.allValid ? '所有必要参数均已通过校验，可以配置测试执行' : '请根据下方提示检查模板标识或参数格式'; $('#resultTime').textContent = new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }); setResultStatus(validation.allValid ? 'success' : 'error', validation.allValid ? '校验通过' : '校验失败'); renderChecks(validation); renderParsed(); setExecutionAvailability(validation.allValid); setWorkflowStep(validation.allValid ? 3 : 2); if (validation.allValid) saveHistory(state.currentUrl, currentPreset().label || '手动链接'); return validation.allValid;
  }
  ...
  function renderChecks(validation) { const templateRow = `<div class="check-row${validation.templateValid ? '' : ' is-error'}"><span class="check-icon">${validation.templateValid ? '✓' : '!'}</span><span>模板标识 <code>temp-lp-jing</code></span><span class="check-status">${validation.templateValid ? 'VALID' : 'MISSING'}</span></div>`; const rows = validation.checks.map((check) => `<div class="check-row${check.valid ? '' : ' is-error'}"><span class="check-icon">${check.valid ? '✓' : '!'}</span><span>${escapeHtml(parameterLabel(check.key))}${check.valid ? '' : ` · ${escapeHtml(check.value || '未填写')}`}</span><span class="check-status">${check.valid ? 'VALID' : 'ERROR'}</span></div>`).join(''); $('#checkList').innerHTML = templateRow + rows; $('#checkSummary').textContent = `${validation.checks.filter((check) => check.valid).length} / ${requiredKeys.length} 必填通过`; }
  function parameterLabel(key) { return config.parameters?.find((item) => item.key === key)?.label || key; }
  function renderParsed() { const entries = Object.entries(state.parsedParams); const visibleEntries = state.showAllParsed ? entries : entries.slice(0, 6); $('#parsedGrid').innerHTML = visibleEntries.map(([key, value]) => `<div class="parsed-item"><span>${escapeHtml(key)}</span><strong title="${escapeHtml(value)}">${escapeHtml(value)}</strong></div>`).join(''); const button = $('#toggleParsedButton'); button.hidden = entries.length <= 6; button.innerHTML = state.showAllParsed ? '收起 <span>‹</span>' : '查看全部 <span>›</span>'; }
  ...
  async function executeOne(index) { await delay(350 + (index % 3) * 180); const successful = state.run.total === 1 || index % 4 !== 0; const duration = `${(1.1 + (index % 5) * 0.37).toFixed(1)}s`; const videoUrl = successful && config.videoBaseUrl ? `${config.videoBaseUrl}${encodeURIComponent(getInput('orderId') || 'test')}-${index}.mp4` : ''; state.run.results.push({ index, successful, duration, videoUrl }); state.run.completed += 1; if (successful) state.run.success += 1; else state.run.failed += 1; }
  ...
  function renderRun() {
    ...
    $('#runTable').innerHTML = run.results.slice().sort((a, b) => a.index - b.index).map((item) => `<div class="run-row"><span class="run-index">#${String(item.index).padStart(2, '0')}</span><span class="run-status ${item.successful ? 'success' : 'failed'}"><i></i>${item.successful ? '成功' : '失败'}</span><span class="run-duration">${item.duration}</span><button class="video-link${item.videoUrl ? '' : ' is-disabled'}" type="button" data-video-url="${escapeHtml(item.videoUrl)}" data-video-title="第 ${item.index} 次测试视频">${item.videoUrl ? '查看视频 ↗' : '视频待接入'}</button></div>`).join('') || '<div class="run-table-empty">测试执行中，明细会实时出现。</div>';
    $$('.video-link:not(.is-disabled)').forEach((button) => button.addEventListener('click', () => openVideo(button.dataset.videoUrl, button.dataset.videoTitle))); const successes = run.results.filter((item) => item.successful); $('#videoCount').textContent = `${successes.length} 条`; $('#videoList').innerHTML = successes.length ? successes.map((item) => `<button class="video-card" type="button" data-video-url="${escapeHtml(item.videoUrl)}" data-video-title="第 ${item.index} 次测试视频"><span class="video-thumb"><span>▶</span></span><span><strong>第 ${item.index} 次测试</strong><small>${item.videoUrl ? '成功视频 · 点击播放' : '视频路径待接入'}</small></span><span class="video-arrow">↗</span></button>`).join('') : '<div class="video-empty">成功后的视频会显示在这里。</div>'; $$('.video-card').forEach((button) => button.addEventListener('click', () => openVideo(button.dataset.videoUrl, button.dataset.videoTitle)));
  }
  ...
  function openVideo(url, title) { $('#videoTitle').textContent = title || '测试视频'; const player = $('#videoPlayer'); const fallback = $('#videoFallback'); if (url) { player.src = url; player.hidden = false; fallback.hidden = true; } else { player.removeAttribute('src'); player.hidden = true; fallback.hidden = false; } $('#videoModal').hidden = false; }
  ...
  function bindEvents() {
    ...
    $('#copyButton').addEventListener('click', copyCurrentUrl); $('#copyInlineButton').addEventListener('click', copyCurrentUrl); $('#openButton').addEventListener('click', () => { if (state.currentUrl) window.open(state.currentUrl, '_blank', 'noopener'); }); $('#toggleParsedButton').addEventListener('click', () => { state.showAllParsed = !state.showAllParsed; renderParsed(); });
