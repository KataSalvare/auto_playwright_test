(() => {
  ...
  const state = { mode: 'auto', presetId: config.presets?.[0]?.id || '', currentUrl: '', validation: null, runs: [], run: null, historyPage: 1, historyPageSize: 5 };
  ...
  function replaceHistoryRuns(payloadRuns) { state.runs = payloadRuns.map((payload) => normalizeRun(payload)); state.run = state.runs[0] || null; state.historyPage = 1; }
  ...
  function resetForm() {
    ...
    $('#advancedToggle').setAttribute('aria-expanded', 'false'); state.currentUrl = ''; state.validation = null; state.parsedParams = {}; state.runs = []; state.run = null; state.historyPage = 1; clearValidation(); clearRun(); setWorkflowStep(1);
  }
  ...
  function clearRun() { $('#resultsEmpty').hidden = false; $('#resultsContent').hidden = true; $('#runProgress').hidden = true; $('#step4').classList.add('is-locked'); setRunStatus('idle', '尚未执行'); const deleteAllButton = $('#deleteAllRecordsButton'); if (deleteAllButton) deleteAllButton.disabled = true; const pagination = $('#historyPagination'); if (pagination) pagination.hidden = true; }
  ...
  function renderRun() {
    const run = state.run; if (!run) { clearRun(); return; } const visibleResults = run.results.filter((item) => !item.deleted); const successCount = visibleResults.filter((item) => item.status === 'success').length; const failedCount = visibleResults.filter((item) => item.status === 'failed').length; const completedCount = run.results.filter((item) => item.status === 'success' || item.status === 'failed').length; const durationEnd = run.completedAt || Date.now(); $('#metricTotal').textContent = visibleResults.length; $('#metricSuccess').textContent = successCount; $('#metricFailed').textContent = failedCount; $('#metricDuration').textContent = completedCount ? `${((durationEnd - run.startedAt) / 1000).toFixed(1)}s` : '—'; $('#runProgressText').textContent = `${completedCount} / ${run.total}`; $('#runProgressBar').style.width = `${run.total ? (completedCount / run.total) * 100 : 0}%`;
    const historyRuns = state.runs.filter((historyRun) => historyRun.results.some((item) => !item.deleted)); const pageCount = Math.max(1, Math.ceil(historyRuns.length / state.historyPageSize)); state.historyPage = Math.min(Math.max(1, state.historyPage), pageCount); const pageStart = (state.historyPage - 1) * state.historyPageSize; const pageRuns = historyRuns.slice(pageStart, pageStart + state.historyPageSize); const canDeleteAll = historyRuns.length > 0 && !state.runs.some((historyRun) => historyRun.status !== 'completed'); $('#runTable').innerHTML = pageRuns.map((historyRun, runIndex) => { const historyResults = historyRun.results.filter((item) => !item.deleted).slice().sort((a, b) => b.index - a.index); const canDelete = Boolean(historyRun.completedAt); const historyStatus = historyRun.status === 'running' ? '执行中' : historyRun.failed ? '部分失败' : '已完成'; const historyStatusClass = historyRun.status === 'running' ? 'pending' : historyRun.failed ? 'failed' : 'success'; const startedAt = historyRun.startedAt ? new Date(historyRun.startedAt).toLocaleString('zh-CN', { hour12: false }) : '时间未知'; const rows = historyResults.map((item) => `<div class="run-row"><span class="run-index">#${String(item.index).padStart(2, '0')}</span><span class="run-status ${statusClass(item.status)}"><i></i>${statusText(item.status)}</span><span class="run-duration">${escapeHtml(item.duration)}</span><span class="run-actions">${renderRunActions(item, canDelete, historyRun.runId)}</span></div>`).join(''); return `<section class="history-run"><div class="history-run-head"><div><strong>执行记录 ${String(pageStart + runIndex + 1).padStart(2, '0')}</strong><span>${escapeHtml(startedAt)} · ${historyRun.total} 次测试</span></div><span class="history-run-status ${historyStatusClass}"><i></i>${historyStatus}</span></div><div class="run-table">${rows}</div></section>`; }).join('') || '<div class="run-table-empty">暂无测试记录。</div>';
    const pagination = $('#historyPagination'); if (pagination) pagination.hidden = historyRuns.length <= state.historyPageSize; const pageInfo = $('#historyPageInfo'); if (pageInfo) pageInfo.textContent = `${state.historyPage} / ${pageCount}`; const previousButton = $('#historyPrevButton'); if (previousButton) previousButton.disabled = state.historyPage <= 1; const nextButton = $('#historyNextButton'); if (nextButton) nextButton.disabled = state.historyPage >= pageCount; const deleteAllButton = $('#deleteAllRecordsButton'); if (deleteAllButton) { deleteAllButton.disabled = !canDeleteAll; deleteAllButton.title = historyRuns.length > 0 && !canDeleteAll ? '执行中的测试完成后才能全部删除' : ''; }
  ...
  async function deleteAllRecords() { const historyRuns = state.runs.filter((historyRun) => historyRun.results.some((item) => !item.deleted)); if (!historyRuns.length) return; if (state.runs.some((historyRun) => historyRun.status !== 'completed')) { showToast('执行中的测试完成后才能全部删除'); return; } if (!window.confirm('确定删除全部测试记录及其视频吗？此操作不可恢复。')) return; try { const response = await fetch('/api/quick-test/runs', { method: 'DELETE' }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || '全部删除失败'); state.runs = []; state.run = null; state.historyPage = 1; clearRun(); showToast('全部测试记录及视频已删除'); } catch (error) { showToast(error instanceof Error ? error.message : '全部删除失败'); } }
